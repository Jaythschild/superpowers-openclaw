...
const ids=start>=0 ? blocks.slice(start).map(b=>b.block_id).filter(id=>id!==token) : [];
const out=[];
for(const id of ids){
...
console.log(JSON.stringify({start, count:ids.length, deleted:out.slice(0,5), last:out.slice(-3)},null,2));
