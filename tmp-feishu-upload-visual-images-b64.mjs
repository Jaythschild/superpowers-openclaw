...
for (const [caption, path] of items) {
  await doc.execute('doc-append', {action:'append', doc_token, content:`\n### ${caption}\n`});
  const b64 = fs.readFileSync(path).toString('base64');
  const res = await doc.execute('doc-upload-image', {action:'upload_image', doc_token, image:`data:image/png;base64,${b64}`, filename: basename(path)});
  results.push({caption, file: basename(path), res: res.details ?? res});
}
