...
fs.writeFileSync('D:/openclaw-stack/workspace/learning/backup-visual-wiki-before-gpt55-premium-reflow-2026-05-19.md', backup?.details?.content||JSON.stringify(backup,null,2),'utf8');
...
for(const p of planned){
  ...
  const up=await doc.execute('upload-reflow-premium-'+String(p.order).padStart(2,'0'),{action:'upload_image',doc_token,image:'data:image/png;base64,'+b64,filename:p.filename,parent_block_id:doc_token,index:p.index});
...
const bad=imgs.filter(x=>!x.token||x.width!==1200||x.height!==420);
...
fs.writeFileSync('D:/openclaw-stack/workspace/learning/gpt55-premium-reflow-verify-2026-05-19.json',JSON.stringify({checks,uploaded:uploaded.sort((a,b)=>a.order-b.order)},null,2),'utf8');
