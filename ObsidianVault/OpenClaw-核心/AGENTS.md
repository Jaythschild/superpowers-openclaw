...
### Feishu Image Batch Directory Guard

For any Feishu document rewrite, image refresh, course update, report upload, or wiki sync that uses local images:

- Create or use exactly one task-specific batch directory under `D:\openclaw-stack\workspace\images\<task-slug>-YYYY-MM-DD-HHmm\`.
- If an image tool first outputs files under `D:\openclaw-stack\state\media\tool-image-generation\`, immediately copy the final selected images into the task-specific batch directory and reference only the copied workspace files from then on.
- Keep that batch's `manifest.json` inside the same directory. Do not upload from a manifest stored elsewhere.
- Markdown drafts must reference images from that same batch directory only, or omit image references and let the upload script insert images from the manifest.
- Before uploading, run:

```powershell
& 'D:\openclaw-stack\workspace\tools\test-image-batch.ps1' -ImageDir '<batch-dir>' -Manifest '<batch-dir>\manifest.json' -Markdown '<optional-md>'
```

- For image replacement or refresh tasks, also pass `-MinLastWriteTime '<task-start-time>'` or `-PreviousManifest '<old-manifest>'` so old images cannot be mistaken for newly generated images.
- If this check fails, do not upload to Feishu and do not claim the images changed.
- Upload success is not visual-change proof. Final verification must include the batch directory, manifest path, file count, sample SHA256 hashes, Feishu revision, imageBlockCount, emptyImageCount, and old-image residual checks.
