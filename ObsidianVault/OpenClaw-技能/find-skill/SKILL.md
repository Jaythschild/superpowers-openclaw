---
name: find-skill
description: Finds and installs Codex-compatible skills for your project. 14 sources, ranked by GitHub stars. Examples: "find a skill for Docker", "search for testing skills", "show all React skills". Supports parameters: limit (how many to show), page (pagination), --all (show all results).
---
...
**Defaults:** limit=5, page=1, **agent=codex** (current agent — Codex).

**Note on `--agent`:** by default only skills compatible with the current agent (Codex) are shown. Use `--agent any` to remove the filter and see the full catalogue, `--agent cursor` to find skills for Cursor only, etc.
